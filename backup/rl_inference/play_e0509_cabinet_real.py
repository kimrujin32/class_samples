...
from dsr_example.simple.new_gripper_drl_controller import GripperController
...
class DoosanCabinetInference(Node):
    ...
    def __init__(self, args: argparse.Namespace):
        ...
        print("[INFO] Gripper Start, Enter를 누르세요...")
        input()
        ...
        if self.gripper_enabled:
            try:
                ...
                self.get_logger().info("Sending a priming command to wake up the gripper...")
                self._wait(5.0)
     
                self.get_logger().info("Sending a priming command ripper.move(700)...")
                self.gripper.move(700) 
                self._wait(5.0)

                self.get_logger().info("Sending a priming command ripper.move(0)...")
                self.gripper.move(0) 
                self._wait(5.0)
                ...
                print(f"_gripper_rad_to_stroke {init_stroke}")
                self.gripper.move(int(init_stroke))


                self.last_gripper_stroke = init_stroke

                print(f"[INFO] Gripper initialized (stroke ~ {init_stroke:.1f}) ended.")
        ...
        print("[INFO] Gripper End, Enter를 누르세요...")
        input()
    ...
    def _apply_action(self, action: np.ndarray):
        ...
        # [중요] 그리퍼 제어 코드를 팔 제어보다 '먼저' 작성하고,
        # 명령을 보냈는지 표시할 플래그 변수를 만듭니다.
        gripper_command_sent = False 

        if self.gripper_enabled and self.gripper is not None:
            # 1. 변환 전/후 값 계산
            raw_rad = self.gripper_joint_rad[2]
            stroke = self._gripper_rad_to_stroke(raw_rad)
            
            # 2. 0.5초 주기 체크
            now = time.time()
            if now - self.last_gripper_cmd_time >= 0.5:
                # 3. 값 변화 체크 (Deadband)
                if self.last_gripper_stroke is None or abs(stroke - self.last_gripper_stroke) > self.gripper_deadband:
                    try:
                        print(f"[GRIPPER] Move: {int(stroke)}")
        ...
        if not gripper_command_sent:
            ...
            # 그리퍼 명령을 보낸 턴에는 로그만 하나 찍어줍니다.
            print("[INFO] Skipped arm command to prioritize gripper.")

        # if self.gripper_enabled and self.gripper is not None:
        #     stroke = self._gripper_rad_to_stroke(self.gripper_joint_rad[2])
        #     if self.last_gripper_stroke is None or abs(stroke - self.last_gripper_stroke) > self.gripper_deadband:
        #         try:
        #             print(f"_gripper_rad_to_stroke: {stroke}")
        #             self.gripper.move(int(stroke))
        #             self.last_gripper_stroke = stroke
        #         except Exception as exc:  # pragma: no cover - 하드웨어 의존
        #             print(f"[WARNING] 그리퍼 명령 실패: {exc}")
    ...
    def spin(self, max_steps: Optional[int] = None):
        ...
        try:
            while rclpy.ok():
                # [추가] ROS2가 밀린 메시지나 콜백을 처리할 수 있게 해줍니다.
